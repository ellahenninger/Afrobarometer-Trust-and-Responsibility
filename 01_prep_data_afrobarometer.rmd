...
afro_wave9$dummy_trust_parl <- ifelse(afro_wave9$Q37B == 2 |
                                      afro_wave9$Q37B == 3, 1,
                                      ifelse(afro_wave9$Q37B > 3, NA, 0))

## Dummy trust president
afro_wave9$dummy_trust_pres <- ifelse(afro_wave9$Q37A == 2 |
                                      afro_wave9$Q37A == 3, 1,
...
##------------------------------------------------------------------------------
x <- afro_wave9_filtered$political_trust_index
afro_wave9_filtered$pol_trust_categorical <- cut(
  x,
  breaks = c(-Inf, 1, 3, Inf),
  labels = c("low", "medium", "high")
)

summary(afro_wave9_filtered$pol_trust_categorical)


##------------------------------------------------------------------------------
## add context variable
##------------------------------------------------------------------------------
afro_wave9_filtered <- afro_wave9_filtered %>%
  group_by(country_alp) %>%
  mutate(
    pol_trust_cntry = weighted.mean(political_trust_index, w = Combinwt_new_hh, na.rm = TRUE)
  ) %>%
  ungroup()
...
## binary outcome (dummy = 1 (responsibility assigned to government))
m1 <- glm(dummy_responsib ~ pol_minus_soc + 
             age + as.factor(gender) + as.factor(education) + as.factor(country_alp),
          data = afro_wave9_filtered, family = binomial(link = "logit"), weights = Combinwt_new_hh)
...
m2 <- glm(dummy_responsib ~ political_trust_index + social_trust_index + 
          ...
          family = binomial(link = "logit"), weights = Combinwt_new_hh)

summary(m2)

m3 <- glm(dummy_responsib ~ pol_trust_categorical + 
            age + as.factor(gender) + as.factor(education) + as.factor(country_alp), 
          data = afro_wave9_filtered, 
          family = binomial(link = "logit"), weights = Combinwt_new_hh)
...
m_int <- glm(dummy_responsib ~ pol_minus_soc*pol_trust_cntry + 
               age + as.factor(gender) + as.factor(education),
             data = afro_wave9_filtered, family = binomial(), 
             weights = Combinwt_new_hh)

summary(m_int)

## multinomial outcome
library(nnet)
afro_wave9_filtered$responsib_merge <- relevel(afro_wave9_filtered$responsib_merge, ref = "The government")
m4 <- multinom(responsib_merge ~ pol_minus_soc + 
                 age + as.factor(gender) + as.factor(education) + as.factor(country_alp), 
               data = afro_wave9_filtered)

stargazer(m4, type = "text", omit = c("country_alp", "education"))
